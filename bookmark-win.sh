...
LOG_FILE=/mnt/c/Users/mihai/Dropbox/Documents/Notite/useful-links.txt
BACKUP_FILE=/mnt/c/Users/mihai/Dropbox/Documents/Notite/backup.useful-links.txt
...
case "$1" in
    ...
    "get" | "-g")
        if [ -z "$2" ]; then
            tail -1 "$LOG_FILE" | clip.exe
            echo "$(powershell.exe Get-Clipboard) copied to clipboard"
        elif [[ "$2" =~ ^[0-9]+$ ]]; then   # check if numerical
            head -n "$2" "$LOG_FILE" | tail -1 | clip.exe
            echo "$(powershell.exe Get-Clipboard) copied to clipboard"
        else
